...
class CoilsModel extends ModelBase {
	static mounted (application, options = {}) {
		...
		const config = require(path.resolve(sequelizercPath, '../', sequelizeConfig.config))[env];
		const sequelizeModelsPath = path.resolve(sequelizercPath, '../', sequelizeConfig['models-path'])
